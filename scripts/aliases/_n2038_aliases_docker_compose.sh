...
dce() {
  ...
  if [ "$#" = "1" ]; then
    ...
    if ! docker-compose exec -T "${__n2038_service_name}" which "${__n2038_shell_name}" > /dev/null 2>&1; then
      __n2038_shell_name="sh"
    fi

    dc exec -it "${__n2038_service_name}" "${__n2038_shell_name}" || { _n2038_unset "$?" && return "$?" || return "$?"; }
  else
    dc exec -it "$@" || { _n2038_unset "$?" && return "$?" || return "$?"; }
  fi
  return 0
}
# ========================================
