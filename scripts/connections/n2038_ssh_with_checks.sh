...
__N2038_PATH_TO_THIS_SCRIPT_FROM_ENVIRONMENT_ROOT="scripts/connections/n2038_ssh_with_checks.sh"
...
#
# Usage: n2038_ssh_with_checks <address[:port]> [password] [arg...]
# Where:
# - "arg": Extra argument to the "ssh" command.
#
# Example: n2038_ssh_without_checks.sh root@archiso:22 some_password
n2038_ssh_with_checks() {
  if [ "$#" -lt 1 ]; then
    _n2038_print_error "Usage: ${c_highlight}n2038_ssh_with_checks <address[:port]> [password] [arg...]${c_return}" || { _n2038_unset "$?" && return "$?" || return "$?"; }
  ...
  if [ -n "${__n2038_password}" ]; then
    ...
    sshpass "-p${__n2038_password}" ssh -p "${__n2038_port}" "$@" "${__n2038_address}" || { _n2038_unset "$?" && return "$?" || return "$?"; }
  else
    ssh -p "${__n2038_port}" "$@" "${__n2038_address}" || { _n2038_unset "$?" && return "$?" || return "$?"; }
  fi
