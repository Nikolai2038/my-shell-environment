...
_n2038_return() {
  ...
  if [ "$(basename "$0" 2> /dev/null)" = "$(eval "basename \"\${_N2038_PATH_TO_THIS_SCRIPT_${_N2038_PATH_TO_THIS_SCRIPT_NUMBER}}\"")" ]; then
    if [ "${N2038_IS_DEBUG_BASH}" = "1" ]; then
...
if [ "$(basename "$0" 2> /dev/null)" = "_n2038_required_before_imports.sh" ]; then
